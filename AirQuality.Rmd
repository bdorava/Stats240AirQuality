...
ozone_plot = full_data %>%
  ...
  labs(title = "Figure 1",
       ...
       color = TeX("Ozone Concentration ($ppb$)"),
       x = TeX("Zonal Component of Velocity (${ms}^{-1}$)"),
       y = TeX("Meridional Component of Velocity ${ms}^{-1}$"))

mean_ozone_plot = grouped_data %>%
  ggplot() + geom_tile(aes(x = xbin, y = ybin, fill = mean_ozone)) + 
  geom_text(aes(x = xbin, y = ybin, label = n_ozone, size = 1/n_ozone)) #Mean Ozone, Numbered Samples


sd_ozone_plot = grouped_data %>%
  ggplot() + geom_tile(aes(x = xbin, y = ybin, fill = sd_ozone)) + 
  geom_text(aes(x = xbin, y = ybin, label = n_ozone, size = 1/n_ozone)) #SD Ozone, Numbered Samples
...
breeze_example_plot = ggplot() +
  ...
  geom_text(aes(x = timestamp, y = atemp-.5, angle = -wdir, size = wspeed),
            ...
            label = "↓",
            color = "red",
            check_overlap = T) #Lake Breeze Example
```
## Introduction

Air pollution has detrimental impacts on community health, especially for vulnerable populations. Some of these impacts can be mitigated by issuing air quality advisories, which are informed by predictions of atmospheric flow. We are interested in analyzing air quality monitoring data from a fixed location 40 miles north of downtown Chicago, and seek to investigate possible correlations between wind velocity and ozone concentrations. From our analysis comparing mean ozone concentrations between samples of similar wind velocity, we find that there is a positive correlation between north-westward blowing wind and ozone concentration.
