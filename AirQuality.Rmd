...
```{r}
stats = mixed_data %>%
  summarise(mu = mean(.$wspeed),
            sigma = sd(.$wspeed))

gnorm(stats$mu, stats$sigma) + geom_vline(xintercept = c(-2,10))
pnorm(c(-2, 10), stats$mu, stats$sigma)

ustats = mixed_data %>%
  summarise(mu = mean(.$u),
            sigma = sd(.$u))

gnorm(ustats$mu, ustats$sigma) + geom_vline(xintercept = c(-8,8))
pnorm(c(-8, 8), ustats$mu, ustats$sigma)

vstats = mixed_data %>%
  summarise(mu = mean(.$v),
            sigma = sd(.$v))

gnorm(vstats$mu, vstats$sigma) + geom_vline(xintercept = c(-8,8))
pnorm(c(-8, 8), vstats$mu, vstats$sigma)

mixed_data = mixed_data %>%
  mutate(high_wind = abs(u) > 8 | abs(v) > 8)

ggplot(mixed_data, aes(x = u, y = v, color = high_wind)) + geom_point()
```

```{r}
mixed_data %>%
  #filter(u < 0) %>%
  filter(year == 2022, month == "May", day == 19, hour > 7) %>%
...
mixed_data %>%
  ...
  filter(wspeed > 10) %>%
  ggplot(aes(x=wdir, y=wspeed, color = source)) + geom_point()
```
