...
ground_data = read_excel("data/2021Chiwaukee.xls", skip=2) %>% 
  ...
  mutate(data_source = "ground") #Add data_source tag
#### Mixed Data ####
...
breeze_example = mixed_data %>%
  filter(year == breeze_example_year,
         month == breeze_example_month,
         day == breeze_example_day)

breeze_ground_example = mixed_data %>%
  filter(year == breeze_example_year,
         month == breeze_example_month,
         day == breeze_example_day) %>%
  filter(data_source == "ground")

breeze_water_example = mixed_data %>%
  filter(year == breeze_example_year,
         month == breeze_example_month,
         day == breeze_example_day) %>%
  filter(data_source == "water")

ggplot() +
  geom_point(aes(x = timestamp, y = atemp, color = ozone),
             breeze_ground_example,
             shape = "square") +
  geom_point(aes(x = timestamp, y = atemp),
             breeze_water_example,
             shape = 3) +
  geom_text(aes(x = timestamp, y = atemp-.5, angle = -wdir, size = wspeed),
            ...
            color = "red",
            check_overlap = T) +
  geom_line(aes(x = timestamp, y = solar/200),
           breeze_ground_example) #TODO Fix solar or remove
```
...
grouped_data %>%
  ...
  geom_point(aes(x=u, y=v, color=ozone), mixed_data %>%
...
mixed_data %>%
  #filter(source == "water") %>%
  ggplot(aes(x = timestamp, y = u, color = data_source)) + geom_point(alpha = 0.5, size = 0.1)
```
